...
/**
 * Encapsulation of binary metadata created from XML to be included as static
 * data in C++ source
 * files.
 *
...
public final class RustMetadataGenerator {
  ...
  /**
   * The metadata type represents the known types of metadata and includes
   * additional information
   * such as the copyright year. It is expected that the generated files will be
   * named after the
  ...
  public enum Type {
    ...
    METADATA(2011, 2025),
    /**
     * The alternate format metadata (expected to be written to
     * alternate_format.[h/cc]).
     */
  ...
  /**
   * Creates a metadata instance that can write C++ source and header files to
   * represent this given
   * byte array as a static unsigned char array. Note that a direct reference to
   * the byte[] is
   * retained by the newly created CppXmlMetadata instance, so the caller should
   * treat the array as
  ...
  /**
   * Writes the source file for the Rust representation of the metadata - a static
   * array
   * containing the data itself, to the given writer. Note that this method does
   * not close the given
   * writer.
   */
  ...
  /**
   * Emits the Rust code corresponding to the binary metadata as a static byte
   * array.
   */
  // @VisibleForTesting
  ...
  /**
   * Emits a single byte in the form 0xHH, where H is an upper case hex digit in
   * [0-9A-F].
   */
